...
class KNNExperiment(experiments.BaseExperiment):
    ...
    def perform(self):
        ...
        if self._details.ds_name == 'bank_marketing':
            params = {'KNN__metric': ['chebyshev'], 'KNN__n_neighbors': np.arange(1, 51, 3),
                      'KNN__weights': ['uniform']}
            complexity_param = {'name': 'KNN__n_neighbors', 'display_name': 'Neighbor count',
                                'values': np.arange(1, 51, 3)}
        ...
        # #
        if self._details.ds_name == 'bank_marketing':
            best_params = {'metric': 'chebyshev', 'n_neighbors': 7, 'weights': 'uniform'}
        elif self._details.ds_name == 'adult_salary':
            best_params = {'metric': 'chebyshev', 'n_neighbors': 7, 'weights': 'uniform'}
