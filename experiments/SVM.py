...
class SVMExperiment(experiments.BaseExperiment):
    ...
    def perform(self):
        ...
        # the various graphs
        #
        if self._details.ds_name == 'bank_marketing':
            best_params_linear = {'C': 0.101, 'class_weight': 'balanced', 'loss': 'squared_hinge',
                               'max_iter': 33, 'tol': 1.00E-08}
            best_params_rbf = {'C': 0.251, 'class_weight': 'balanced', 'decision_function_shape': 'ovo',
                                'tol': 0.07000001}
        elif self._details.ds_name == 'adult_salary':
            best_params_linear = {'C': 0.001, 'class_weight': 'balanced', 'loss': 'squared_hinge',
                               'max_iter': 42, 'tol': 1.00E-08}
            best_params_rbf = {'C': 0.251, 'class_weight': 'balanced', 'decision_function_shape': 'ovo',
                               'tol': 0.06000001}

        # Linear SVM
        params = {'SVM__max_iter': iters, 'SVM__tol': tols, 'SVM__class_weight': [{1: 10}],
                  'SVM__C': C_values}
        ...
        best_params = experiments.perform_experiment(
            ...
            params, complexity_param=complexity_param, seed=self._details.seed,
        ...
        # params = {'SVM__C': C_values, 'SVM__class_weight': ['balanced'], 'SVM__tol': tols, 'SVM__decision_function_shape': ['ovo', 'ovr']}
        # complexity_param = {'name': 'SVM__C', 'display_name': 'Penalty', 'values': np.arange(0.001, 2.5, 0.25)}
        # #
